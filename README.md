...
| カバレッジ地図 | Leaflet.js ベースのインタラクティブ世界地図。ステータスごとに色分け表示。Canvas レンダリングにより10万件以上のタイルでも高速描画 |
| テーブルビュー | タイル一覧。ステータス・座標でフィルタリング可能。1ページ250件のページング表示で大量タイルでも軽快に動作 |
...
| シーン取込み | 実際の撮像フットプリント（軌道方向に回転したポリゴン）を取り込み、タイルごとのカバレッジ率を精密に計算 |
...
│   │   ├── models.py            # ORM モデル（Tile, Satellite, OrbitalPass, Order, Scene）
...
│   │   │   ├── scenes.py        # CRUD /api/scenes（フットプリント取込み・カバレッジ再計算）
...
│   │       └── coverage.py      # Shapely によるポリゴン交差計算・coverage_pct 更新
...
```
例: GRUS-1A/B/C (70 km) の場合
  → 70 km ÷ 111 km/° ≈ 0.63°
  → きりの良い最大値: 0.5°
  → 0.5°×0.5° グリッドを生成（全球 ~259,200 タイル、陸域 ~130,000 件）
```

> **タイル数について**  
> 0.5° グリッドは陸域だけで約13万件になります。初期化・表示が遅い場合は  
> `--tile-size 1.0` などで粗いグリッドに変更してください。
...
`main` ブランチにプッシュすると GitHub Actions が自動的にフロントエンドを
...
以降は `main` ブランチへのプッシュごとに自動デプロイされます。
...
GitHub Pages 上ではバックエンドが存在しないため、`config.js` が
自動的にモックモードを有効化します（コード変更不要）。
...
| Method | Path | 説明 |
|--------|------|------|
| GET | `/api/scenes` | シーン一覧（`?order_id=1&satellite_id=2` でフィルタ可） |
| POST | `/api/scenes` | シーン取込み。フットプリントを GeoJSON Polygon で指定。交差タイルの coverage_pct を自動再計算 |
| GET | `/api/scenes/{id}` | シーン詳細 |
| DELETE | `/api/scenes/{id}` | シーン削除（影響タイルのカバレッジを再計算） |
...
衛星センサーの撮像範囲は**軌道方向に平行な回転ポリゴン**で、緯度経度線には平行ではありません。
「オーダーが完了した = タイルが完全にカバーされた」という単純なバイナリ管理は精度が低く、
再撮影判断に誤りが生じます。

シーン取込み機能（`POST /api/scenes`）を使うと：

1. 実際に撮像されたフットプリント（GeoJSON Polygon）を登録
2. **Shapely** による厳密なポリゴン交差計算で、フットプリントと重なる全タイルの `coverage_pct` を自動更新
3. `coverage_pct ≥ 80%` のタイルは自動的に `COMPLETED` へ、それ未満は `IN_PROGRESS` のまま継続推薦
...
UI（Orders タブ → "Ingest Scene" ボタン）から以下の方法で入力できます：

| 方法 | 説明 |
|------|------|
| **4コーナー座標** | UL / UR / LR / LL の緯度・経度を直接入力 |
| **GeoJSON ペースト** | `{"type":"Polygon","coordinates":[...]}` を貼り付け |

GeoJSON 座標は **[経度, 緯度]** 順（GeoJSON 規約）です。

### DB 再初期化について

`coverage_pct` 列と `scenes` テーブルは新規追加です。
既存の DB（`world_cover.db`）がある場合は再初期化が必要です：
...
オーダーに紐付けてシーンを取り込むと、オーダーは自動的に **COMPLETED** に遷移します。
シーンを取り込まず手動で COMPLETED にした場合は、タイルのステータスのみ更新されます（`coverage_pct` は変化なし）。
...
| 空間計算 | Shapely 2.x（フットプリント×タイル交差計算・カバレッジ率算出） |
