...
def _apply_tle(sat: Satellite, line1: str, line2: str) -> None:
    """Update TLE fields and re-parse epoch."""
...
def create_satellite(body: SatelliteCreate, db: Session = Depends(get_db)):
    try:
...
def patch_satellite(sat_id: int, body: SatellitePatch, db: Session = Depends(get_db)):
    sat = db.get(Satellite, sat_id)
    if not sat:
        raise HTTPException(status_code=404, detail="Satellite not found")
    ...
    # If TLE lines are being updated, validate and re-parse epoch
